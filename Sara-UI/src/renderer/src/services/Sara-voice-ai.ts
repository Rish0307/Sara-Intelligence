...
export class GeminiLiveService {
  ...
  async connect(): Promise<void> {
    ...
    const storedPersonality = window.electron?.ipcRenderer
      ? await window.electron.ipcRenderer.invoke('get-personality')
      : null
...
# 🔮 Sara — YOUR INTELLIGENT COMPANION
You are **Sara**, a high-performance AI agent. You don't just talk; you **execute**.
...
- **Example:** "Sara, find my code and send it to Harsh on WhatsApp."
...
`
    ...
    const finalSystemInstruction = SARA_SYSTEM_INSTRUCTION + contextPrompt
          ...
          tools: [
            {
              functionDeclarations: [
                  ...
                  parameters: {
                    ...
                    properties: {
                      ...
                      custom_text: {
                        type: 'STRING',
                        description:
                          'If rewriting text, generate a highly cinematic, hacker-style headline to inject into the website. (e.g., "SARA HAS TAKEN OVER", or whatever the user requested).'
                      }
                    },
                    required: ['url', 'mode']
                  }
                ...
                },
                {
                  ...
                  description:
                    'ACTION: Spawns the Sara Live Forge and generates a full, highly animated, real-time website using Tailwind CSS and GSAP. Use this when the user asks you to build a landing page, a portfolio, a 3D site, or a complex web interface.',
                  parameters: {
                    type: 'OBJECT',
                    properties: {
                      prompt: {
                ...
                },
                {
                  ...
                  description:
                    'Instantly locks the Sara OS system, disconnects the AI, and returns the user to the secure biometric lock screen. Use this strictly when the user says "Lock the system", "Lock down", or "Activate Sentry Mode".',
                  parameters: {
                    type: 'OBJECT',
                    properties: {}
                  }
                },
                {
                  ...
                  parameters: {
                    ...
                    properties: {
                      goal: {
                        type: 'STRING',
                        description: 'The exact natural language goal or instruction to pass to the Python agent.'
                      }
                    },
                    required: ['goal']
                  }
        ...
        if (data.toolCall) {
          ...
          await Promise.all(
            functionCalls.map(async (call: any) => {
              ...
              } else if (call.name === 'run_advanced_python_agent') {
                try {
                  const response = await fetch('http://127.0.0.1:5000/api/execute', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ goal: call.args.goal })
                  });
                  const data = await response.json();
                  result = data.result || data.error || 'Python Agent completed the task.';
                } catch (e: any) {
                  result = 'Error connecting to the Python Server: ' + e.message;
                }
              } else {
