...
export default function LockScreen({ onUnlock }: LockScreenProps) {
      ...
      try {
        ...
        if (detection) {
          ...
          if (isFaceSetup) {
            ...
            const isMatch = window.electron?.ipcRenderer
              ? await window.electron.ipcRenderer.invoke('verify-vault-face', descriptorArray)
              : true // bypass when electron unavailable
  ...
  const processPin = async (currentPin: string) => {
    if (!window.electron?.ipcRenderer) {
      // No Electron IPC - just unlock with any 4-digit PIN
      triggerAccessGranted()
      return
    }
    if (needsPinSetup) {
    ...
    >
      ...
      <div className="absolute bottom-6 flex flex-col items-center gap-1 z-50">
        <span className="text-[9px] font-mono tracking-widest text-zinc-600 uppercase">
          Sara Intelligence Security Engine V3.5
        </span>
